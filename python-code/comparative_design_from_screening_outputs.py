...
SCREENING_CSV = os.path.join(SCRIPT_DIR, 'screening_output',
    ...
    if 'design' in df.columns:
        for design in df['design']:
            if '-rule' in design:
                ...
                raise ValueError(f"Cannot parse design '{design}'. Expected 'S2-12-rule2'.")
    elif {'strategy', 'slots', 'rule'}.issubset(df.columns):
        for _, row in df.iterrows():
            candidates.append((f"S{int(row['strategy'])}-{int(row['slots'])}", int(row['rule'])))
    else:
        raise KeyError(f"Expected 'design' column or 'strategy'/'slots'/'rule'. "
...
def load_weekly_ov(output_dir: str, tag: str, rule: int) -> np.ndarray:
    ...
    if 'OV' in df.columns:
        return df['OV'].values
    elif 'elAppWT' in df.columns and 'urScanWT' in df.columns:
...
def paired_diff_ci(arr_i: np.ndarray, arr_j: np.ndarray, alpha_per: float) -> dict:
    ...
    hw     = t_crit * se
    return {'mean_diff': mean, 'hw': hw,
            'lower': mean - hw, 'upper': mean + hw, 'n': n}


def make_label(tag: str, rule: int) -> str:
    parts = tag.split('-')
    N_str = parts[1] if len(parts) >= 2 else '?'
    return f"N={N_str}, R={rule}"
...
def plot_ci_chart(chart_rows: list[dict], title: str,
                  xlabel: str, filepath: str, conf_pct: float,
                  is_pairwise: bool = False):
    """
    Forest-plot style CI chart with CI bounds and verdict labels.
    chart_rows: list of dicts with keys: label, mean_diff, lower, upper, significant
                and optionally 'i', 'j' for pairwise verdict text.
    """
    n = len(chart_rows)
    fig, ax = plt.subplots(figsize=(12, 1.6 * n + 2.5))
    fig.patch.set_facecolor('#F7F7F7')
    ax.set_facecolor('#F7F7F7')

    color_better = '#27AE60'   # green  – significantly better  (diff < 0, CI fully negative)
    color_worse  = '#C0392B'   # red    – significantly worse   (diff > 0, CI fully positive)
    color_nosig  = '#2980B9'   # blue   – no significant difference
    ...
    for row, ypos in zip(chart_rows, y_positions):
        mean, lo, hi = row['mean_diff'], row['lower'], row['upper']

        # Determine colour and verdict text
        if hi < 0:
            color = color_better
            if is_pairwise:
                # label is "N=12, R=2 vs N=14, R=2" – first part is i
                parts   = row['label'].split(' vs ')
                verdict = f"{parts[0].strip()} significantly better"
            else:
                verdict = "Significantly better than base"
        elif lo > 0:
            color = color_worse
            if is_pairwise:
                parts   = row['label'].split(' vs ')
                verdict = f"{parts[1].strip()} significantly better"
            else:
                verdict = "Significantly worse than base"
        else:
            color   = color_nosig
        ...
        # CI bound labels  [lo, hi]  just inside the caps
        ax.text(lo - x_range * 0.01, ypos + 0.32, f"{lo:+.4f}",
                ha='right', va='bottom', fontsize=7.5, color=color, alpha=0.85)
        ax.text(hi + x_range * 0.01, ypos + 0.32, f"{hi:+.4f}",
                ha='left',  va='bottom', fontsize=7.5, color=color, alpha=0.85)

        # Verdict label to the right of the CI
        ax.text(x_max + x_range * 0.04, ypos, verdict,
                va='center', fontsize=8.5, color=color, fontstyle='italic')
    ...
    # Extend x-axis right to make room for verdict text
    ax.set_xlim(x_min - x_range * 0.15, x_max + x_range * 0.55)

    better_patch = mpatches.Patch(color=color_better, label='Significantly better (CI fully < 0)')
    worse_patch  = mpatches.Patch(color=color_worse,  label='Significantly worse  (CI fully > 0)')
    nosig_patch  = mpatches.Patch(color=color_nosig,  label='No significant difference')
    ax.legend(handles=[better_patch, worse_patch, nosig_patch],
              fontsize=8.5, loc='lower left', framealpha=0.85)
...
def compare_vs_base(candidates: list, base_ov: np.ndarray) -> list:
    ...
    print(f"{'='*65}")
    print(f"{'i':<18}  {'Xi – Xs':>9}  {'Half length':>11}  {'CI-':>8}  {'CI+':>8}  Significant?")
    print("-" * 75)
    ...
    for tag, rule in candidates:
        ...
        print(f"{label:<18}  {res['mean_diff']:>9.4f}  {res['hw']:>11.4f}  "
        ...
        if is_sig:
            significant.append({'tag': tag, 'rule': rule, 'label': label, 'ov': cand_ov})
...
def pairwise_comparison(significant_candidates: list):
    ...
    print(f"  Per-CI level           : {conf_pct:.4f}%")
    print(f"{'='*65}")
    print(f"{'i':<18}  {'j':<18}  {'Xi – Xj':>9}  {'Half length':>11}  "
    ...
    for sys_i, sys_j in itertools.combinations(significant_candidates, 2):
        ...
        li, lj  = sys_i['label'], sys_j['label']
        is_sig  = (res['upper'] < 0) or (res['lower'] > 0)
        sig_str = "YES" if is_sig else "no"

        print(f"{li:<18}  {lj:<18}  {res['mean_diff']:>9.4f}  {res['hw']:>11.4f}  "
              f"{res['lower']:>8.4f}  {res['upper']:>8.4f}  {sig_str}")
...
def main():
    ...
    print(f"\nBase ({BASE_TAG}, rule {BASE_RULE}): {len(base_ov)} weeks, "
          f"mean OV = {base_ov.mean():.4f}")
